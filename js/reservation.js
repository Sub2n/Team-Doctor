...
navigator.geolocation.getCurrentPosition(function(position) {
    ...
    coord = new daum.maps.LatLng(position.coords.latitude, position.coords.longitude);
});
...
window.setTimeout(function() {
    ...
    if (status === daum.maps.services.Status.OK) {
        console.dir(result[0].address);
        local = result[0].address.region_3depth_name.slice(0, 3);
        }
    };
...
// 키워드 검색을 요청하는 함수입니다
function searchPlaces(hospital) {

    let keyword = local + hospital;
